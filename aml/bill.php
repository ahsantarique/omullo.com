	    ...
	    <th class="tg-b44r">
	    	<select class="pid">
	    		...
	    		<?php 
	    			...
	    			$sql = "SELECT * FROM jobs where ul=1 and subjid!=0 group by ecode ";
 ...
 $('#example3').on('click', '.filter', function (event) {
   ...
   //alert(datetype);      
	if(datetype=='startdate'|| datetype=='delivered'){
		$.post('action/billfilterwithdate.php',{pid:pid,datetype:datetype,std:std,end:end},function(data){
      	$.get("action/billfilterwithdate.php?pid="+pid+"&datetype="+datetype+"&std="+std+"&end="+end,function(data){
      		//alert(datetype);
      		$('tbody').html(data);
      	})
      })
	}else{
		$.post('action/billfilter.php',{pid:pid},function(data){
      	$.get("action/billfilter.php?pid="+pid,function(data){
      		//alert('ok');
      		$('tbody').html(data);
      	})
      })
