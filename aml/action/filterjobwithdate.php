...
$end=$_GET['end'];
   	include('../config.php');	
	$sql1 = "SELECT * FROM jobs where ('$cid'='' || ccode='$cid') and ('$mid'='' || mcode='$mid') and ('$sid'='' || sid='$sid') and ('$pid'='' || ecode='$pid') and ('$ul'='' || ul='$ul') and (DATE($datetype) >= CAST('$std' AS DATE) AND  DATE($datetype) <= CAST('$end'AS DATE)) ";
