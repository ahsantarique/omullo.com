...
<table class="tg table table-bordered table-striped" id="example1">
	<thead>
		<tr>
	    <th class="tg-b44r"></th>
	    <th class="tg-b44r"></th>
	    ...
	    <th class="tg-b44r" colspan="2">
	    	<input type="radio" name="date" value="incoming">Incoming
	    	<input type="radio" name="date" value="delivery">Delivery
	    </th>
  ...
  <tbody>
	...
	if($userCat=='Admin' || $userCat=='Manager' || $userCat=='In-Charge'){
		$sql = "SELECT * FROM jobs where jobs.ul=0 ";
	}else{
		$sql = "SELECT * FROM jobs where ecode='$usercode' and jobs.ul=0 ";
	}
...
<script type="text/javascript">
 $(document).ready(function(){
  ...
  $('#example1').on('click', '.filter', function (event) {
  	//alert("hello");
      ...
      $.post('action/filterjob.php',{cid:cid,mid:mid,sid:sid,pid:pid,ul:ul},function(data){
      	$.get("action/filterjob.php?cid="+cid+"mid="+mid+"sid="+sid+"pid="+pid+"ul="+ul,function(data){
      		alert(cid);
      		$('tbody').html(data);
      	})
      })
      ...
      // $.post('queryPages/showComment.php',{assignmentId:assignmentId},function(data){
      //   $.get("queryPages/showComment.php?assignmentId="+assignmentId,function(data){
      //     //alert(divId);
      //     $(divId).html(data);
      //   });
      // });
