...
<table class="tg table table-bordered table-striped table-responsive" id="example1">
	<thead>
		<tr>
	    <th class="tg-b44r"></th>
	    <th class="tg-b44r"></th>
 ...
 $(document).ready(function(){
  ...
  $('#example1').on('click', '.filter', function (event) {
	...
	if(datetype=='startdate'|| datetype=='enddate'){
		$.post('action/filterjobwithdate.php',{cid:cid,mid:mid,sid:sid,pid:pid,ul:ul,datetype:datetype,std:std,end:end},function(data){
      	$.get("action/filterjobwithdate.php?cid="+cid+"&mid="+mid+"&sid="+sid+"&pid="+pid+"&ul="+ul+"&datetype="+datetype+"&std="+std+"&end="+end,function(data){
      		//alert(datetype);
      		$('tbody').html(data);
      	})
      })
